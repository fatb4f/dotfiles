...
hookrail_registry_export() {
  ...
  (cd "$(hookrail_repo_root)" && cue export ./cue/registry "$overlay_file" -e "$expression" --out json)
}
...
hookrail_run_registry_resolve() {
  ...
  if hookrail_registry_export "$overlay_file" 'resolution' | jq '
    {
      status: .status,
      query: .query
    }
      ...
      if .status == "selected" then
        ...
        + (
          if .status == "blocked" then
            {
              blockedCandidateIDs: [.blockedCandidates[].node.id]
            }
          else
            {}
          end
