...
#ContextSelectionCutoverPropertyCatalog: close({
	...
	properties: [ID=#ContextSelectionCutoverPropertyID]: #ContextSelectionCutoverProperty & {
		id: ID
	}
})
...
#ContextSelectionCutoverQualificationReport: close({
	...
	propertyReport: close({
		...
		results: [...close({
			propertyID: #ContextSelectionCutoverPropertyID
			status:     "passed"
		})]
...
contextSelectionCutoverFixtures: {
	boundary: {
		inside: {
			request:  #SelectionCutoverRequest
			snapshot: #SelectionCutoverSnapshot
			selected: [{kind: "member", id: "member.inside"}]
			files:    ["src/inside"]
		}
		outsideMember: {
			request:  #SelectionCutoverRequest
			snapshot: #SelectionCutoverSnapshot
			selected: [{kind: "member", id: "member.outside"}]
			files:    []
		}
		outsideFile: {
			request:  #SelectionCutoverRequest
		...
		repositoryRoot: {
			...
			snapshot: #SelectionCutoverSnapshot
			selected: [
				{kind: "member", id: "member.inside"},
				{kind: "member", id: "member.outside"},
			]
	...
	committed: {
		...
		Proposal=proposal: #ContextRootProposal & {
			...
			snapshotID:   Projection.graph.snapshotID
			memberIDs:    []
			namespaceIDs: []
			pathPrefixes: []
		}
		...
		evaluation: #ContextCommittedSelectionEvaluation & {
			request:             Request
			proposal:            Proposal
			policy:              Policy
...
// Additional fixtures repair the original property runner so each named
// strategy executes a distinct mutation rather than sharing one expression.
contextSelectionPropertyFixtures: {
	...
	endpointReversed: {
		snapshot: #SelectionPropertySnapshot & {
			...
			relationships: {
				"rel.reversed": {
					subject:     {kind: "namespace", id: "namespace.fixture"}
					predicate:   "contains"
					object:      {kind: "module", id: "module.fixture"}
					evidenceIDs: []
				}
			}
		}
		predicates: ["contains"]
		previous: [{
			entity:      {kind: "module", id: "module.fixture"}
			distance:    0
			direction:   "root"
			predecessor: null
		}]
