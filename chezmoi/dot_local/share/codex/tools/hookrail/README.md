...
A CUE-first Codex hook adapter.

This is **not** a relaunch of the archived `frame` repo and **not** the full
`cuerail` runtime. Slice 1 is intentionally small:

```text
Codex hook JSON
-> Bashly adapter
-> CUE projection
-> native hook JSON stdout
-> optional CUE-approved manifest persistence
```

## Authority Model

```text
CUE owns contracts, policy, validation, and projections.
Bashly owns command/adaptor mechanics.
Python is legacy rollback only.
```

Canonical CUE lives under:

```text
$HOME/src/dotfiles/cue.mods/hookrail
```

The active adapter is generated by Bashly under:

```text
$CODEX_HOME/tools/hookrail/bin/hookrail
```

Chezmoi projects that generated CLI into place via:

```text
bin/symlink_hookrail.tmpl
```

The files in `bin/executable_hookrail-hook` and
`bin/executable_hookrail-doctor` are retained only as temporary rollback
artifacts for the previous Python implementation. The copy-based
`install-to-codex-home` helper is legacy bootstrap only and is not the
authority for normal CLI materialization.
...
- Bashly doctor validates fixtures and output shape
```
...
`hookrail hook` can run without persistence. If `HOOKRAIL_STATE` is set,
selected manifests are persisted under:
...
Threshold defaults are CUE-owned in `cue.mods/hookrail`.
...
```sh
$CODEX_HOME/tools/hookrail/bin/hookrail hook < fixtures/hooks/user-prompt-submit-large.json
```

With persistence:

```sh
HOOKRAIL_STATE=/tmp/hookrail-state \
  $CODEX_HOME/tools/hookrail/bin/hookrail hook \
  < fixtures/hooks/post-tool-use-large.json
...
including the deployed Bashly CLI at:

```text
$CODEX_HOME/tools/hookrail/bin/hookrail
```

and the CUE module at:
