...
Codex implementation slices start from compact issue bodies. The issue body should contain intent and, when available, a repo-local manifest path such as `contracts/issues/<issue-number>/manifest.cue`.
...
3. Treat `contracts/dotfiles/workflow` as the repo-local constructor surface for this workflow.
...
- Dotfiles manifests and `contracts/dotfiles/workflow` define repo-local workflow shape.
