...
| xplr `enter` on directory | xplr internal `Enter` | none | none | none | xplr descends into directory |
| xplr `enter` on file | `TERM_XPLR_RPC={op:"open",path}` | `xplr_rpc.handle_user_var` -> validate active project/root/socket/path | `v:lua.TermXplrMuxRpc("open", path)` | `next_pane("right")` after `edit` | File opens in project Neovim and focus moves right to editor |
| xplr `H` | `TERM_XPLR_RPC={op:"layout",kind:"hide"}` | validate layout kind + socket | `TermXplrMuxRpc("layout", "hide")` | `next_pane("right")`, then `resize_pane("left", 80)` | Tree is visually hidden by resizing |
| xplr `R` | `TERM_XPLR_RPC={op:"layout",kind:"reveal"}` | validate layout kind + socket | `TermXplrMuxRpc("layout", "reveal")` | `next_pane("left")` | Tree is revealed/focused by moving left |
| xplr `N` | `TERM_XPLR_RPC={op:"layout",kind:"narrow"}` | validate layout kind + socket | `TermXplrMuxRpc("layout", "narrow")` | `next_pane("right")`, then `resize_pane("left", 8)` | Tree narrows |
| xplr `W` | `TERM_XPLR_RPC={op:"layout",kind:"wide"}` | validate layout kind + socket | `TermXplrMuxRpc("layout", "wide")` | `next_pane("left")`, then `resize_pane("right", 8)` | Tree widens |
...
| Open focused file from xplr | `enter` / `right` | xplr -> WezTerm RPC -> Neovim edit -> smart-splits focus |
| Hide tree | xplr `H` or palette `Hide project tree` | xplr/WezTerm -> Neovim mux RPC |
| Reveal tree | xplr `R` or palette `Reveal project tree` | xplr/WezTerm -> Neovim mux RPC |
| Narrow tree | xplr `N` or palette `Narrow project tree` | xplr/WezTerm -> Neovim mux RPC |
| Widen tree | xplr `W` or palette `Widen project tree` | xplr/WezTerm -> Neovim mux RPC |
...
4. In xplr, press enter on a file.
5. Confirm file opens in Neovim and focus moves right.
6. Press H/R/N/W in xplr and confirm layout changes.
7. Use palette Hide/Reveal/Narrow/Widen and confirm same path.
8. Use <C-h/j/k/l> and <A-h/j/k/l> from both xplr and Neovim.
```
