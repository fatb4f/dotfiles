...
| Root schema fixture | `nodes/workspace/projections.cue` exports `rootSchemaDerivedFixture` | Proves downstream fragment conformance, interop state, allowed CUE/Go flow adapter/MCP/prompt relations, typed evidence relations, and invalid Go adapter-owned policy drift classification. |
...
| Root CUE SSOT | Root schema owns type contracts, fragment contracts, relations, interop state, and evidence. | `cue/agentnode/schema.cue` now exports root-owned fragment, relation, interop, evidence, and admissibility contracts; `AGENTS.cue` declares `schemaSource` as `root-cue-ssot`. | Implemented structurally for AgentNode CUE fixtures; the planned Go layer is modeled as an API adapter around `cuelang.org/go/tools/flow`, not as policy authority. | Medium | When the adapter appears, require it to consume these root-shaped fragments, adapt CUE flow transitions, and emit root-shaped evidence only. |
| Relation admissibility | A fragment is admissible only when both type and relation are valid. | `rootSchemaDerivedFixture.admissibility` exports all four classifications: admissible fragment, architectural drift, schema gap, and reject. | Implemented as typed fixture classification, not runtime enforcement. | Medium | Keep Go adapter-owned policy classified as drift unless a root CUE relation authorizes it. |
...
| Relation validity is first-class. | Proven for CUE fixtures | `rootSchemaDerivedFixture.relations` exports allowed CUE contract to Go flow adapter, Go flow adapter to CUE-shaped evidence, Go flow adapter to `cuelang.org/go/tools/flow`, root MCP to authorization evidence, prompt projection to agent consumer, and disallowed Go adapter-owned hidden policy. |
| Invalid Go adapter-owned policy is flagged. | Proven for CUE fixtures | `rootSchemaDerivedFixture.admissibility` classifies `hidden-go-policy` as `architectural-drift` because its type is valid but its relation is invalid. |
...
| `cue export ./nodes/workspace -e rootSchemaDerivedFixture --out json` | Exported downstream fragment contracts, interop state, allowed relation edges including the Go adapter to `cuelang.org/go/tools/flow`, and admissibility classifications including Go adapter-owned policy drift. |
...
9. Treat valid type plus invalid relation as architectural drift; the current fixture flags Go adapter-owned hidden policy this way.
